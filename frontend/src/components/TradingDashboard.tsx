...
const TradingDashboard: React.FC = () => {
  const { isLiveMode, exchangeStatus } = useTradingContext();
  const [showDebugger, setShowDebugger] = useState(false);
  const [errorCount, setErrorCount] = useState(0);

  const getConnectionStatus = () => {
    const connected = Object.values(exchangeStatus).filter(status => status).length;
    const total = Object.keys(exchangeStatus).length;
