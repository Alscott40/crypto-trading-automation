...
interface CustomWindow extends Window {
  logError?: (type: 'error' | 'warning' | 'info', message: string, details?: string, source?: string) => void;
}

  useEffect(() => {
    // Set up global error logging function
    (window as CustomWindow).logError = (type: 'error' | 'warning' | 'info', message: string, details?: string, source?: string) => {
    ...
    // Log initial message
    (window as CustomWindow).logError?.('info', 'Error logging system initialized', 'Ready to capture trading errors');

    return () => {
      delete (window as CustomWindow).logError;
    };
  }, []);
  ...
  const clearLogs = () => {
    setLogs([]);
    (window as CustomWindow).logError?.('info', 'Error logs cleared');
  };
